...
flowchart LR
    A(Scelta del contenuto) --> B{Oggetto della guida}
        ...
        H --> I{Guida accurata?}
            I --> |Sì| V(Produzione delle grafiche)
        ...
        N--> O{Risultati coerenti?}
            O -->|Sì| P(Scrittura contenuto)
                P --> V(Produzione delle grafiche)
            O -->|No| M(Raccolta informazioni)

        B --> |Missioni| Q(Raccolta Informazioni)
            Q --> R(Esecuzione della missione)
            R --> S(Scrittura del contenuto)
            S --> T{C'è una quest successiva?}
                T -->|Sì| Q(Raccolta Informazioni)
                T -->|No| U(Revisione e Correzione)
                    U --> V(Produzione delle grafiche)
    V --> Y(Formattazione Finale)
